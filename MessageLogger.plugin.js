...
class MessageLogger {
  ...
  load() {
    ...
    if (fs.existsSync(path.join(BdApi.Plugins.folder, 'MessageLoggerV2.plugin.js'))) return;
    request('https://gitlab.com/_Lighty_/bdstuff/-/raw/master/public/plugins/MessageLoggerV2.plugin.js', (error, response, body) => {
      try {
        ...
        fs.writeFile(path.join(BdApi.Plugins.folder, 'MessageLoggerV2.plugin.js'), body, () => {});
      } catch (e) {
